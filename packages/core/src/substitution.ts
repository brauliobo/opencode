    ...
    return Service.of({
      substitute: Effect.fn("Substitution.substitute")(function* (input) {
        ...
        for (const match of fileMatches) {
          ...
          const read = yield* fs.readFileString(resolved).pipe(Effect.exit)
          if (Exit.isFailure(read) && missing !== "empty") {
            return yield* new FileReferenceError({ source: configSource, token, resolved, cause: read.cause })
          }
          const content = Exit.isSuccess(read) ? read.value : ""
